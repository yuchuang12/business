...
The repository layout and git history are portable. Adding a remote later requires no content migration: configure the remote, push the existing default branch, enable branch protection, and change the project resource or execution mode through a new ADR.
